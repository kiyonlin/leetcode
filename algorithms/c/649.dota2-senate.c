...
char* predictPartyVictory(char* senate)
{
    ...
    bool* alive = (bool*)malloc(sizeof(bool) * len);
    memset(alive, true, len);
    bool firstRound = true;
    int r = 0, d = 0; // count of d and r
    int killed = 0; // killed >= 0 r can kill d; killed <= 0 d can kill r;
    int i = 0;
    while ((r > 0 && d > 0) || firstRound) {
        if (i == len) {
            i %= len;
            firstRound = false;
        }
        if (*(alive + i)) { // i is alive
            if (*(senate + i) == 'R') {
                if (killed >= 0) {
                    d--;
                } else {
                    *(alive + i) = false; // was killed
                }
                if (firstRound) {
                    r++;
                }
